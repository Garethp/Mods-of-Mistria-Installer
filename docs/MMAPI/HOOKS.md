...
Most shipped hooks are dispatched from engine seams. A few are [emitted directly by the MMAPI runtime](SEAMS.md#runtime-hooks). Mods consume both in exactly the same way.
...
The shipped catalog currently declares **137 hooks**, fed by **153 seams**, **16 engine fixes**, and **1 call rewrite**. The [Catalog](CATALOG.md) gives each one its own page.
...
Put registrations behind the latch shown in [Mod Anatomy](MOD_ANATOMY.md#the-registered_hooks-latch). Registration is memory-only and safe during boot. Reading config, touching live engine state, and performing file IO are not, so do that later from the handler or a queued tick.

> [!IMPORTANT]
> Use the directive named by the hook's kind. A mismatched registration still lands, but the shipped dispatcher skips it, so the handler never runs.

## The Four Kinds

Start from what the handler is allowed to do:

| Intent | Kind | Directive | Handler | Return contract |
| ------ | ---- | ------------- | ------- | --------------- |
...
> [!IMPORTANT]
> `undefined` means **decline**. A filter keeps the current value, a guard allows, and an override defers. Events ignore every return value. For a guard, only the Boolean value `false` vetoes, and numeric zero still allows.

Most filters replace a value by returning one. Two shipped hooks, `combat.tarball_grid` and `ui.item_node`, are declared **in place** instead. Their handlers mutate the first `value` argument because the dispatch site deliberately discards the return, and both receive `undefined` as `ctx`. Their hook pages call this out prominently.
...
Any non-`undefined` result claims the override chain, including `false`, zero, and an empty string. Later override handlers do not run. The individual hook contract determines what the engine does with the winning value, which does not always mean "the engine action happened."

## Override Contention

Only one override answer wins, so every shipped override hook declares a **contention** class:

- **exclusive** means the hook expects at most one mod to override it. A rival registration logs a warning naming both mods. Both handlers remain registered, and dispatch order decides which one gets the first opportunity to answer.
- **claim-scoped** means multiple mods are expected to coexist. Each handler must return `undefined` for targets it does not own. A rival registration logs an informational message explaining that contract.

`object.interact` is claim-scoped, so each mod recognizes its own objects and defers everywhere else. An exclusive override normally represents one indivisible engine decision where rival answers genuinely conflict.

## Registration Options And Dispatch Order

All four directives accept the same optional `opts` struct:
...
Alias resolution happens during registration only. Dispatch and introspection functions do not translate an old name, so use the canonical name everywhere. Unknown-name warnings are also once per name globally, so the mod that registers the name first receives the warning and later consumers may not see it in their own logs.
...
Custom mod-published hooks do not belong in `requires_hooks`, because they are not part of MOMI's catalog. See [Publishing Custom Hooks](#publishing-custom-hooks).
...
The individual hook page documents frequency when it matters. Treat a log-and-flush probe as temporary on a hot hook, because flushing every dispatch turns a useful test into repeated file IO.
...
If the proof line is absent, read every `WARN` in the same file. The common causes are an unknown hook name, a kind mismatch, an exact duplicate, or an exception inside the handler. Unknown and mismatched registrations remain in the registry, while exact duplicates are skipped.
...
| `mmapi_hook_kind(name)` | The declared kind, and therefore the directive to use. |
...
| `mmapi_hook_fired_count(name)` | How many times the hook dispatched while [debug](DEBUG.md) was enabled. Returns 0 in a session that never enabled debug. |
| `mmapi_hook_liveness()` | `{ enabled, counts, silent, undeclared }`. `silent` lists catalog hooks that never dispatched while debug was enabled, and `undeclared` lists counted names outside the catalog, custom mod-emitted hooks included. Many hooks fire only in specific game contexts, so a silent hook is a lead to probe, not an alarm. |
...
- The directive matches the kind on the hook page.
...
- A filter or override returns `undefined` when it does not want to answer.
- A guard returns Boolean `false` only when it intends to veto.
...
The dispatchers are ordinary GML functions, so a mod may publish its own hooks for other mods to handle:
...
Namespace the hook to the publishing mod, such as `my_mod.item_charged`, and document the same contract as a shipped hook, covering exact kind, timing, value, `ctx`, return behavior, and frequency. Consumers must use the matching directive.
...
Do not put custom names in `requires_hooks`, because that field proves a MOMI catalog hook exists, not that another mod is installed. When the publisher is optional, an absent publisher simply means the handler never fires. If the publisher is required, coordinate that dependency outside the hook catalog.
