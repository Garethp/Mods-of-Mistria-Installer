...
- [game.day_started](game.day_started.md) - This is the poll-side observation of the day number changing: it fires one frame later (after the end-of-day autosave), also fires after a cross-day save load, and never fires for the day a session starts in. Prefer `game.new_day` for daily resets and grants.
