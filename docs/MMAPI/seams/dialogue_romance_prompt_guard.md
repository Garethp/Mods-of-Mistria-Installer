...
| **ctx built** | `{ npc_id, speaker, path }` from the open textbox menu's speaker (`requested_speaker`, falling back to `current_speaker`) and `driver` |
| **Marker** | `mmapi_dialogue_run_romance_prompt_guards` |

## The Edit

The pristine closure styles a pink prompt and then locks it when `!ARI.disable_break_ups && (ARI.spouse() != undefined || ARI.fiance() != undefined)` holds. The replace captures that condition into a local, and only when it is false dispatches the guard with a ctx built from `ANCHOR.get_menu(Menu.Textbox)`. The speaker is read as `requested_speaker` with `current_speaker` as the fallback. When a line changes speakers the engine defers its `set_speaker` call into the portrait animation chain, so `current_speaker` can still name the previous line's speaker while the prompts are styled, and `requested_speaker` already names the asking speaker. The speaker's `identity` string resolves to an `NpcId` through `try_string_to_npc_id`, which yields undefined for cameo speakers, and `path` reads the driver's `conversation_name`. A guard veto sets the same local, and the original four lock statements run off it unchanged.
...
- Both speaker fields can be undefined for info line asks, and `driver` is undefined outside a driven conversation. The ctx fields degrade to undefined rather than failing, and a crash inside the ctx build is caught and counts as allow.
