...
The placement is the point. A renderer that just scrolled into view is reactivated carrying its last `sprite_index`, and the MMAPI begin_step tick (which runs before this cull, at `Game.gml` `step_begin`) cannot have refreshed it yet. Re-applying per-instance sprite and visual state from this hook closes that 1-frame scroll-in lag, so a mod's sprite override never lags a frame on scroll-in.
