...
| **Marker** | `mmapi_furniture_place_guard` |
...
When any guard returns `false`, the injected line runs `return undefined;`. `write_furniture_to_location` returns `undefined`, nothing is placed, and the item stays in the player's hand. Every other return allows. With zero handlers the guard check early-outs on an empty registry, leaving pristine behavior.
