...
| **kind** | A hook's dispatch style, one of event, filter, guard, or override. It fixes what the handler receives and what its return value means. |
| **event** | A kind. The handler reacts to a moment, and the return value is ignored. An individual hook may document context fields handlers can mutate. |
| **filter** | A kind. The handler normally receives a value and returns a replacement, or `undefined` to keep it. A hook declared **in place** instead requires mutation and discards the return. |
| **guard** | A kind. The handler returns the Boolean value `false` to veto an action, and `undefined`, `true`, numeric zero, other values, and errors all allow it. |
| **override** | A kind. The handler returns a value to replace the engine's answer, and the first non-`undefined` result wins. |
| **directive** | The `mmapi_*` function that registers a handler for a kind, one of `mmapi_on`, `mmapi_filter`, `mmapi_guard`, and `mmapi_override`. |
...
| **seam** | A verified edit to one engine script that makes one or more hooks fire. MOMI authors and ships seams, and mod packages never carry them. |
| **seam catalog** | The TOML file, `seams.toml`, that declares every hook and seam. Also called the catalog. |
| **provider** | What makes a hook fire. Most are `seam`-provided, while a `runtime` hook is emitted by MMAPI itself without an engine edit. |
...
| **locator** | The full rule that identifies an edit site, either an exact context/anchor or a structural target. |
...
| **marker** | A unique identity token present in a staged seam or engine fix. It catches collisions and is not an idempotency guard. |
| **depends_on** | Catalog entry ids that must apply first. MOMI uses them to compute a stable topological order. |
...
| **vanilla** | The game's own unmodded behavior and data, as distinct from the pristine source, which is the unmodified archive. |
| **staged tree** | The in-memory future GML tree after earlier edits have composed but before the archive rebuild is written. |
| **compile gate** | The checker pass over the staged framework and each mod. A shared failure aborts, while an individual mod failure skips that mod. |
| **boot** | A mod's top-level loading phase while the game is still starting. It is memory-only, with no live player or room and no file IO. |
